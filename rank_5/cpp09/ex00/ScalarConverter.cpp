...
void ScalarConverter::convert(const std::string& stringLiteral)
{
	...
	/* when the conversion is completed successfully, the p pointer
	will point to the null character of 'f' char are only valid options */
	if (p && *p == 'f'
	&& (trimmedString.find_first_of('.') != std::string::npos
		|| convertNum != convertNum
		|| convertNum == positiveInf
		|| convertNum == negativeInf))
	{
			++p;
	}
...

	{
		...
		/* limits */
		const double charMin = static_cast<double>(std::numeric_limits<char>::min());
		const double charMax = static_cast<double>(std::numeric_limits<char>::max());
