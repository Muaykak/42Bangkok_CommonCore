...
_isEpollout(false)
{
	_requestBuffer.reserve(HTTP_RECV_BUFFER * 2);
	_requestBufferOffset = 0;
}
...
{
	if (requestData.getProcessStatus() != NO_STATUS
	&& requestData.getProcessStatus() != READING_HEADER
	&& requestData.getProcessStatus() != READING_REQUEST_LINE)
	{
		std::cout << "====================================" << std::endl;
		//print request line
		{
...
bool Http::hasResponseList() const
{
	return (!_httpResponseList.empty());
}
...
void Http::cgiChildProcessNoRequestBody(HttpRequest& requestData, int pipeForCgiStdOut[2])
{
	try 
	{
		...
		{
			envData().assignEnv("PATH_INFO", "");
			envData().assignEnv("PATH_TRANSLATED", "");
...
{
	if (httpRequest.getProcessStatus() == NO_STATUS)
		httpRequest.setProcessStatus(READING_REQUEST_LINE);

	if (httpRequest.getProcessStatus() != READING_REQUEST_LINE)
		return ;
	
	{
		std::time_t currentTime = std::time(NULL);
		if (std::difftime(currentTime, httpRequest.getStateTimeStamp()) >= WEBSERV_HTTP_REQUEST_TIMEOUT_SECOND)
			generate4xx5xxErrorReponse(httpRequest, 408, false, "Http::Request Timeout::took too much time to send the request header");
	}
	...
	if (httpReq.requestData.method.empty())
	{
		...
		size_t	endLinePos;
		if (findNextCRLF(_requestBuffer, endLinePos, currIndex) == false)
			generate4xx5xxErrorReponse(httpRequest, 400, false, "the \'\\r\' must always followed by \'\\n\'");

		// doesn't find any endline delimiter
		// will process later
		if (endLinePos == _requestBuffer.npos){
			/* really need to check if the request line is tooo long */
			if (_requestBuffer.size() - currIndex >= MAX_HTTP_REQUEST_LINE_LENGTH)
				generate4xx5xxErrorReponse(httpReq, 414, false, "REQUEST LINE IS TOO LONG");
		...
		if (endLinePos - currIndex >= MAX_HTTP_REQUEST_LINE_LENGTH)
			generate4xx5xxErrorReponse(httpReq, 414, false, "REQUEST LINE IS TOO LONG");

		static CharTable sp(" ", true);
		// find pos any whitespaces but not '\n'
		size_t	pos = sp.findFirstCharset(_requestBuffer, currIndex, endLinePos - currIndex);
		...
		if (httpReq.requestData.method.empty() || httpTokenChar().isMatch(httpReq.requestData.method) == false)
		...
		pos = sp.findFirstCharset(_requestBuffer, currIndex, endLinePos - currIndex);
	...
	httpReq.setProcessStatus(READING_HEADER);
	return ;
}
...
void	Http::parsingHttpHeader(size_t& currIndex, size_t& reqBuffSize)
{
	...
	while (httpRequest.getProcessStatus() == READING_HEADER)
	{
		/* check timeout */
		{
			std::time_t currentTime = std::time(NULL);
		...
		if (endLinePos == currIndex)
		{
			...
			httpRequest.setProcessStatus(VALIDATING_REQUEST);
		...
		httpRequest.requestData.headerBufferCount += endLinePos - currIndex;

		if (httpRequest.requestData.headerBufferCount > MAX_HTTP_HEADER_BUFFER_LIMIT)
			generate4xx5xxErrorReponse(httpRequest, 431, false, "name in header field too long");
		...
		/* header name must not too long */
		if (tempFieldName.size() > MAX_HTTP_HEADER_LINE_LENGTH)
			generate4xx5xxErrorReponse(httpRequest, 431, false, "name in header is too long");
		...
		if ((endLinePos - currIndex) > MAX_HTTP_HEADER_LINE_LENGTH)
				generate4xx5xxErrorReponse(httpRequest, 431, false, "value in header is too long");
...
void	Http::generate3xxRedirectResponse(HttpRequest& requestData, unsigned int statusCode)
{
	...
	// status message
	targetResponse.statusLine->second = getStatusCodeMessage(statusCode);
...
void	Http::generate4xx5xxErrorReponseChildProcess(HttpRequest& requestData, unsigned int errorStatusCode, bool keepAfterResponse, const std::string& throwMsg)
{
	...
	if (targetResponse.keepAfterResponse == true)
	...
	std::string statusMessageTemp = getStatusCodeMessage(errorStatusCode);
...
void	Http::generate4xx5xxErrorReponse(HttpRequest& requestData, unsigned int errorStatusCode, bool keepAfterResponse, const std::string& throwMsg)
{
	...
	if (errorStatusCode == 405 && requestData.serverData.targetServerPtr && requestData.serverData.targetLocationBlockPtr)
	{
		const std::vector<std::string>* foundAllowedMethodPtr = requestData.serverData.targetServerPtr->getLocationData(requestData.serverData.targetLocationBlockPtr, "allowed_methods");
	...
	// set status message
	targetResponse.statusLine->second = getStatusCodeMessage(errorStatusCode);
...
static bool convertAuth(const std::string& authStr, std::string& host, int& portNum)
{
	std::string portStr;
...
void Http::checkHost(HttpRequest& requestData, std::string& hostStr)
{
	if (hostStr.empty() || hostStr.size() > 255)
		generate4xx5xxErrorReponse(requestData, 400, false, "Invalid::URI Authority::hostname length is invalid");

	bool lastCharWasDot = true;
	size_t labelLength = 0;

	for (size_t i = 0; i < hostStr.length(); ++i)
	{
		char c = hostStr[i];

		if (c == ';')
			generate4xx5xxErrorReponse(requestData, 400, false, "Invalid::URI Authority::Semicolon is forbidden in hostname");

		if (c == '.')
		{
			if (lastCharWasDot || (i > 0 && hostStr[i - 1] == '-'))
				generate4xx5xxErrorReponse(requestData, 400, false, "Invalid::URI Authority::Malformed dots/hyphens placement");
			lastCharWasDot = true;
			labelLength = 0;
		}
		...
		{
			if (lastCharWasDot && c == '-')
				generate4xx5xxErrorReponse(requestData, 400, false, "Invalid::URI Authority::Label cannot start with a hyphen");
			if (++labelLength > 63)
				generate4xx5xxErrorReponse(requestData, 400, false, "Invalid::URI Authority::Label component too long (>63)");
			lastCharWasDot = false;
		}
		else
			generate4xx5xxErrorReponse(requestData, 400, false, "Invalid::URI Authority::Illegal characters detected in hostname");
	}

	if (hostStr[hostStr.length() - 1] == '-')
		generate4xx5xxErrorReponse(requestData, 400, false, "Invalid::URI Authority::hostname cannot end with a hyphen");

	hostStr = stringToLower(hostStr);
...
void	Http::validateRequestBufferSelectServer(HttpRequest& requestData,const std::string& authStr)
{
	...
	if (convertAuth(authStr, host, portNum) == false)
		generate4xx5xxErrorReponse(requestData, 400, false, "Invalid::URI Authority::");
	checkHost(requestData, host);
	{
		int realHostPortNum;
		std::string realHost;
		if (convertAuth(requestData.requestData.headerField.find("host")->second, realHost, realHostPortNum) == false)
			generate4xx5xxErrorReponse(requestData, 400, false, "Http:: Host invalid value");
		checkHost(requestData, realHost);
		if (realHost != host || realHostPortNum != portNum)
		{
			generate4xx5xxErrorReponse(requestData, 403, false, "Http:: Host and URI not matched::" + host + "  " + realHost);
		}
	}
	...
	// now we check the host, if it is ip or server_name (or whatever it's called)
	{
		...
		else
		{
			...
			//	if (c == '.')
			//	{
			//		if (lastCharWasDot || (i > 0 && host[i - 1] == '-'))
			//			generate4xx5xxErrorReponse(requestData, 400, false, "Invalid::URI Authority::Malformed dots/hyphens placement");
			//		lastCharWasDot = true;
			//		labelLength = 0;
			//	}
			...
			while (serverVecIt != ptr->end() && requestData.serverData.targetServerPtr == NULL)
			{
				...
				else
				{
					...
					while (tempNameVecIt != serverVecIt->getServerNameVec().end())
					{
						...
						if (stringToLower(*tempNameVecIt) == host)
						{
							requestData.serverData.targetServerPtr = &(*serverVecIt);
							break ;
						}
...
void	Http::requestLineCheckRequestTargetPathCheck(HttpRequest& requestData)
{
	...
	bool need303redirect = requestData.targetData.targetPath.find("//") == std::string::npos ? false : true;
	...
	if (need303redirect)
	{
		...
		generate3xxRedirectResponse(requestData, 303);
	}
...
void	Http::checkExpectBody(HttpRequest& requestData)
{
	...
	{
		/* if expect found assume that the request must have the body */
		if (requestData.bodyData.readingRequestBodyPtr == NULL)
			generate4xx5xxErrorReponse(requestData, 400, false, "Http:: Expect: request with no body should not have Expect:");
...
{
	std::map<std::string, std::string>& target = requestData.requestData.headerField;

	std::map<std::string, std::string>::const_iterator content_length = target.find("content-length");
	std::map<std::string, std::string>::const_iterator tranfer_encoding = target.find("transfer-encoding");

	if (content_length != target.end() && tranfer_encoding != target.end())
	...
	if (content_length == target.end() && tranfer_encoding == target.end())
	{
		/* here means no body also */
		requestData.bodyData.readingRequestBodyPtr = NULL;
		requestData.bodyData.body_size = 0;
		requestData.bodyData.readBody = false;
		return;
	}

	/* if content_length is found */
	if (content_length != target.end())
	{
		size_t convertNum = 0;
		...
		requestData.bodyData.readingRequestBodyPtr = &Http::readingContentLengthBody;
		requestData.bodyData.body_size = convertNum;
	...
	else
	{
		...
		{
			//std::cout << "valueVec[" << i << "]: \"" << valueVec[i] << "\"\n";
			if (stringToLower(valueVec[i]) == "chunked")
				isFoundChunked = true;
		}
		...
		requestData.bodyData.readingRequestBodyPtr = &Http::readingTranferEncodingBody;
		requestData.bodyData.body_size = 0;
		requestData.bodyData.readBody = true;
	}
	...
	if (requestData.bodyData.client_max_body_size == 0)
		requestData.bodyData.client_max_body_size = std::numeric_limits<size_t>::max();

	//std::cout << "CLIENT MAX BODY: " << requestData.bodyData.client_max_body_size << std::endl;

	if (content_length != target.end() && requestData.bodyData.body_size > requestData.bodyData.client_max_body_size)
...
void	Http::checkMethod(HttpRequest& requestData)
{
	...
	if (found == false)
	{
		generate4xx5xxErrorReponse(requestData, 405, false, "Http::method not allowed in this location::" + requestData.requestData.method);
	}
}
...
void	Http::checkCgiPath(HttpRequest& requestData)
{
	...
	if (foundCgiPass != NULL)
	{
		...
		std::map<std::string, std::vector<std::string> >	tempCgiPassMap;
		{
			std::vector<std::string>::const_iterator vecIt = foundCgiPass->begin();
			std::vector<std::string>::const_iterator nextIt = vecIt;
			while (vecIt != foundCgiPass->end())
			{
				if ((*vecIt)[0] != '.')
					generate4xx5xxErrorReponse(requestData, 500, false, "Internal Error::CgiPass on this location block is misconfigured");

				nextIt = vecIt + 1;
				while (nextIt != foundCgiPass->end())
				{
					if ((*nextIt)[0] == '.')
						break ;
					++nextIt;
				...
				tempCgiPassMap.insert(std::make_pair(*vecIt, std::vector<std::string>(vecIt + 1, nextIt)));
				if (tempCgiPassMap[*vecIt].size() < 1)
					generate4xx5xxErrorReponse(requestData, 500, false, "Internal Error::CgiPass on this location block is misconfigured");
				vecIt = nextIt;
			}
		}
		...
		std::string tempTofil = requestData.targetData.targetPath.substr(tempPath.size() > requestData.targetData.targetPath.size() ? requestData.targetData.targetPath.size() : tempPath.size());
		...
		while (!tempTofil.empty())
		{
			...
			else 
			{
				...
				{
					/* check if that CGI pass allow this request method to be cgi */
					std::vector<std::string>& target = it->second;
					if (target.size() > 1)
					{
						bool found = false;
						for (size_t i = 1; i < target.size(); i++)
						{
							if (target[i] == requestData.requestData.method)
								found = true;
						}
						if (found == false)
							break ;
	...
	// /* if cgiPath is relative path must turn it into absulute path */
	if (requestData.cgiData.cgiPath[0] != '/')
	{
		std::string pwdString = envData().findValue("PWD");
		if (!pwdString.empty())
			requestData.cgiData.cgiPath = pwdString + '/' + requestData.cgiData.cgiPath;
	}
...
{
	if (requestData.bodyData.readingRequestBodyPtr == NULL)
		generate4xx5xxErrorReponse(requestData, 400, false, "Content-Type: multipart/form-data REQUEST must have the body");
	...
	// open the file for writing now
	{
		...
		if (fd < 0)
		{
			generate4xx5xxErrorReponse(requestData, 500, false, "POST method to non-cgi with Content-Type: multipart/form-data::open() error::" + std::string(std::strerror(errno)));
		}
...
// void Http::handleUploadOctetStream(HttpRequest& requestData)
// {
// 	/*
...
// 	if (stat(requestData.targetData.combinedPath.c_str(), &fileStat) == 0)
// 	{
// 		/* if the target is a directory we don't need to upload anything */
// 		if (S_ISDIR(fileStat.st_mode))
// 		{
// 			requestData.bodyData.tempRequestBodyFileNum = tempFileManager().generateNewTempFile();
// 			std::string tempFilePath = TEMP_FILE_DIR + toString(requestData.bodyData.tempRequestBodyFileNum);
...
// 			if (fd < 0)
// 			{
// 				generate4xx5xxErrorReponse(requestData, 500, false, "HTTP::validating::POST method to non-cgi to directory Path::open() error::" + std::string(std::strerror(errno)));
// 			}
...
void Http::handleNormalUpload(HttpRequest &requestData, std::pair<std::string, std::vector<std::pair<std::string, std::string> > >& outPair)
{
	/* if no body then we just skip to FINISED READ BODY*/
	if (requestData.bodyData.readingRequestBodyPtr == NULL)
	{
		requestData.setProcessStatus(FINISHED_READ_BODY);
		return ;
	...
	/* for normal upload that is not multiform part data if the path is directory then sohudl consider not putting any */
	struct stat fileStat;
	std::memset(&fileStat, 0, sizeof(fileStat));
	if (stat(requestData.targetData.combinedPath.c_str(), &fileStat) == 0)
	{
		if (S_ISDIR(fileStat.st_mode))
		{
			/* if post normal upload to directory would discard the body*/
			requestData.bodyData.isUseTempFile = false;
			requestData.bodyData.readBody = true;
			requestData.bodyData.discardBody = true;
			requestData.setProcessStatus(READ_BODY);
		...
		if (S_ISREG(fileStat.st_mode))
			...
			generate4xx5xxErrorReponse(requestData, 403, true, "Forbidden::stat()::the target must be regular file!");
	}
...
void Http::handleUploadPostRequest(HttpRequest& requestData)
{
...
{
	/* post method can be empty so what can we do here */
	if (requestData.bodyData.readingRequestBodyPtr == NULL)
	{
		requestData.setProcessStatus(FINISHED_READ_BODY);
		return ;
	...
	//	ErrMsg += strerror(errno);
	//	if (errno == EACCES)
	//		generate4xx5xxErrorReponse(requestData, 403, false, ErrMsg);
	//	generate4xx5xxErrorReponse(requestData, 404, false, ErrMsg);
	//}
...
void	Http::validateRequestData(HttpRequest& requestData)
{
	...
	try
	{
		...
		*/
		...
		checkConnectionHeader(requestData);
	...
	{
		if (e.statusCode() < 400 && e.statusCode() >= 300)
			return ;
		else
			throw ;
	}
...
void Http::readingContentLengthBody(HttpRequest& requestData)
{
	size_t	readBodyAmount;
	{
		...
		size_t availableSize = _requestBuffer.size() - _requestBufferOffset;
		if (availableSize == 0)
			return ;
	...
	if (requestData.bodyData.discardBody == true)
	{
		...
		_requestBufferOffset += readBodyAmount;
	...
	if (_requestBufferOffset > 0)
	{
		_requestBuffer.erase(0, _requestBufferOffset);
		_requestBufferOffset = 0;
	}
...
int Http::readTransferEncodingChunkFinished(HttpRequest& requestData, size_t& endLinePos)
{
	...
	if (endLinePos == std::string::npos)
	{
		if (requestData.bodyData.chunkedBodyHasTrailerHeader == true)
		{
			...
			*/
			if (availableSize > MAX_HTTP_HEADER_LINE_LENGTH + 1)
			{
				if(requestData.bodyData.discardBody)
	...
	else
	{
		...
		*/
		if (endLinePos == _requestBufferOffset)
		{
			_requestBufferOffset += 2;
			requestData.setProcessStatus(FINISHED_READ_BODY);
			return (1);
		}
		else
		{
			...
			{
				std::string headerLineStr = _requestBuffer.substr(_requestBufferOffset, endLinePos - _requestBufferOffset);
				...
				_requestBufferOffset = endLinePos + 2;
				return (2);
			}
			else
			{
...
int	Http::readTransferEncodingChunk(HttpRequest& requestData, size_t& endLinePos)
{
	if (requestData.bodyData.curr_body_read >= requestData.bodyData.body_size)
	{
		if (requestData.bodyData.curr_body_read > 0)
		{
			size_t availablesize = _requestBuffer.size() - _requestBufferOffset;
			if (availablesize >= 2 && requestData.bodyData.skipCRLFtranferencoding == false)
			{
				if (_requestBuffer.compare(_requestBufferOffset, 2, "\r\n") == 0)
				{
					_requestBuffer.erase(0, 2);
					requestData.bodyData.skipCRLFtranferencoding = true;
				}
				else
				{
					if(requestData.bodyData.discardBody)
						generate4xx5xxErrorReponse(requestData, 400, false, "Request Body Chunked::missing CRLF");
			...
			else if (availablesize == 1 && _requestBuffer[_requestBufferOffset] == '\r')
				return (1);
		...
		requestData.bodyData.skipCRLFtranferencoding = false;

		std::string hexString = _requestBuffer.substr(_requestBufferOffset, endLinePos - _requestBufferOffset);

		// if (hexString.empty() || hexChar()[hexString[0]] == false)
		// {
		// 	if(requestData.bodyData.discardBody)
		// 		generate4xx5xxErrorReponse(requestData, 400, false, "Request Body Chunked::invalid transfer-encoding");
		...
		if (hex_to_size_t(hexString, hexNum) == false)
		{
			...
			if(requestData.bodyData.discardBody)
				generate4xx5xxErrorReponse(requestData, 400, false, "Request Body Chunked::wrong hex format::" + toString(hexNum));
		...
		//std::cout << "HEX NUM: " << hexNum << '\n' << "curr BODY SIZE: " << requestData.bodyData.body_size << '\n';
		...
		*/
		{
			...
			{
				if(requestData.bodyData.discardBody)
					generate4xx5xxErrorReponse(requestData, 413, false, "Request Body Chunked::Too large::" + toString(hexNum));
				requestData.bodyData.writeBodyFile.clear();
				if (requestData.bodyData.isUseTempFile)
					tempFileManager().removeTempFile(requestData.bodyData.tempRequestBodyFileNum);
				else
	...
	size_t	readBodyAmount;
	{
		...
		// then if the readBody amount is more than the _requestBuffer.size() then
		if (readBodyAmount > availableSize)
		{
			readBodyAmount = availableSize;
		}
	}
...
void Http::readingTranferEncodingBody(HttpRequest& requestData)
{
	size_t	endLinePos;
	int ret;
	...
	while (true)
	{
		if (requestData.bodyData.chunkedBodyIsFinished == true)
			ret = readTransferEncodingChunkFinished(requestData, endLinePos);
		else
			ret = readTransferEncodingChunk(requestData, endLinePos);
...
void Http::readingRequestBody(HttpRequest& requestData)
{
	...
	// 	// std::cout << "currentTime: " << buffer << '\n';
	// 	std::time_t stamp = requestData.getStateTimeStamp();
	// 	localTime = std::localtime(&stamp);
	// 	std::strftime(buffer, sizeof(buffer), "%Y-%m-%d %H:%M:%S", localTime);
	// 	// std::cout << "state Time Stamp: " << buffer << '\n';
	// 	if (std::difftime(currentTime, httpRequest.getStateTimeStamp()) >= WEBSERV_HTTP_REQUEST_TIMEOUT_SECOND)
	// 		generate4xx5xxErrorReponse(httpRequest, 408, false, "Http::Request Timeout::took too much time to send the request header");
	// }

	if (requestData.bodyData.readingRequestBodyPtr)
		(this->*requestData.bodyData.readingRequestBodyPtr)(requestData);
	else
	{
		requestData.setProcessStatus(FINISHED_READ_BODY);
		return ;
...
void Http::handleCGIrequest(HttpRequest &requestData)
{
	...
	else
	{
		...
		if (fcntl(pipe_out[0], F_SETFL, O_NONBLOCK | O_CLOEXEC) != 0)
		{
			tempFileManager().removeTempFile(requestData.bodyData.tempRequestBodyFileNum);
			kill(pid, SIGTERM);
			waitpid(pid, NULL, 0);
		...
		if (requestData.bodyData.readingRequestBodyPtr)
		{
			if (fcntl(pipe_in[1], F_SETFL, O_NONBLOCK | O_CLOEXEC) != 0)
			{
				tempFileManager().removeTempFile(requestData.bodyData.tempRequestBodyFileNum);
				kill(pid, SIGTERM);
				waitpid(pid, NULL, 0);
	...
	if (requestData.bodyData.multiformData->state == FORMDATA_STATUS_READING_BODY)
	{
		...
		while (true)
		{
			...
			if (writeBuffer.empty() == false)
			{
				...
				if (writeAmount >= static_cast<ssize_t>(writeBuffer.size()))
				{
					writeBuffer.clear();
				}
				else
...
void Http::send408()
{
	try
	{
		...
		generate4xx5xxErrorReponse(httpRequest, 408, false, "Request Timeout");
		/* code */
	}
...
void Http::directRequestProcess(HttpRequest requestData)
{
	try 
	{
		if (requestData.responseTargetPtr == NULL)
		{
			_httpResponseList.push_back(HttpResponse());
			requestData.responseTargetPtr = &_httpResponseList.back();
		}
...
void Http::readFromClient()
{
	...
	// std::cout << "CURR STATE: ";
	// switch (httpRequest.getProcessStatus())
	// {
	// 	case NO_STATUS:
	// 		std::cout << "NO_STATUS" << '\n';
	// 		break ;
	// 	case READING_REQUEST_LINE:
	// 		std::cout << "READING_REQUEST_LINE" << '\n';
	// 		break ;
	// 	case READING_HEADER:
	// 		std::cout << "READING_HEADER" << '\n';
	// 		break ;
	...
	// 		std::cout << "READ_BODY" << '\n';
	// 		break ;
	// 	case FINISHED_READ_BODY:
	// 		std::cout << "FINISHED_READ_BODY" << '\n';
	// 		break ;
	// 	default:
	// 		std::cout << '\n';
	// }
	...
	//std::cout << "readFromClient !: " << count++ << "     READAMOUNT: " << readAmount << '\n';
	if (readAmount == 0)
	{
