/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   redirect.c                                         :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: muaykak <muaykak@student.42.fr>            +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/06/28 10:21:44 by srussame          #+#    #+#             */
/*   Updated: 2025/06/30 11:44:05 by muaykak          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../include/minishell.h"

static int	file_in(int *fd, t_redirect *re)
{
	if (*fd > 2)
	{
		close(*fd);
		*fd = 0;
	}
	if (access(re->value, F_OK))
		return (1);
	if (access(re->value, R_OK))
		return (2);
	*fd = open(re->value, O_RDONLY);
	return (0);
}

static int	file_out(int *fd, t_redirect *re)
{
	if (*fd > 2)
		close(*fd);
	if (re->type == WRITE_FILE)
		*fd = open(re->value, O_WRONLY | O_TRUNC | O_CREAT, 0644);
	else if (re->type == APPEND_FILE)
		*fd = open(re->value, O_WRONLY | O_APPEND | O_CREAT, 0644);
	if (access(re->value, W_OK) || is_directory(re->value))
	{
		if (*fd > 2)
			close(*fd);
		return (2);
	}
	return (0);
}

static void	fd_err(int fd_in, int fd_out, int status, t_redirect *re)
{
	if (fd_in > 2)
		close(fd_in);
	if (fd_out > 2)
		close(fd_out);
	if (is_directory(re->value))
		ft_printf(2, "minishell: %s: Is a directory\n", re->value);
	else if (status == 1)
		ft_printf(2, "minishell: %s: No such file or directory\n", re->value);
	else if (status == 2)
		ft_printf(2, "minishell: %s: Permission denied\n", re->value);
	*get_code() = 1;
}

bool	find_fd(t_redirect *re, int *fd_in, int *fd_out)
{
	int		status;

	status = 0;
	*fd_out = 1;
	*fd_in = 0;
	while (re)
	{
		if (re->type == HERE_DOC || re->type == READ_FILE)
			status = file_in(fd_in, re);
		else if (re->type == APPEND_FILE || re->type == WRITE_FILE)
			status = file_out(fd_out, re);
		if (status > 0)
		{
			fd_err(*fd_in, *fd_out, status, re);
			return (false);
		}
		re = re->next;
	}
	return (true);
}

bool	redirect(t_process *proc, bool child_flag)
{
	int		fd_in;
	int		fd_out;

	if (find_fd(proc->redirect, &fd_in, &fd_out) == false)
	{
		if (child_flag == true && proc->cmd[0] == NULL)
			*get_code() = 0;
		return (false);
	}
	if (fd_in != 0 && fd_in > 0)
	{
		dup2(fd_in, 0);
		close(fd_in);
	}
	if (fd_out != 1 && fd_out > 1)
	{
		dup2(fd_out, 1);
		close(fd_out);
	}
	return (true);
}
