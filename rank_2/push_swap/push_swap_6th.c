...
int	check_correct_pos(t_list *top_stack, t_list *target)
{
	...
	if (show_int(target) == high_num(top_stack))
		return (1);
	if (/*show_int(before_target) < show_int(target) 
	&& */show_int(after_target) > show_int(target))
		return (1);
	return (0);
	//
//	if (show_int(target) == low_num(top_stack))
//	{
//		if (show_int(before_target) == high_num(target))
//			return (1);
//	}
//	if (show_int(target) == high_num(top_stack))
//		return (1);
//	if (show_int(before_target) == high_num(top_stack))
//		return (1);
//	if (show_int(before_target) <= show_int(target))
//		return (1);
//	return (0);
...
//	if (show_int(target) == high_num(top_stack))
//	{
//		if (target->next && show_int(target->next) == low_num(top_stack))
//			return (1);
//		else if (!target->next && show_int(top_stack) == low_num(top_stack))
//			return (1);
//	}
//	if (show_int(target) == low_num(top_stack))
//		return (1);
//	if (target->next && show_int(target->next) == low_num(top_stack))
//		return (1);
//	if (!target->next && show_int(top_stack) == low_num(top_stack))
//		return (1);
//	if (target->next && show_int(target) <= show_int(target->next))
//		return (1);
//	if (!target->next && show_int(target) <= show_int(top_stack))
//		return (1);
//	return (0);
}
...
t_list	*find_close_unsorted(t_list *top_stack_a, t_list **fake_list)
{
	...
	while (fake_run)
	{
		...
		(*((t_list **)(fake_run->content)))) < 
		travese_dist(top_stack_a, target))
...
t_list	*find_b(t_list *stack_a, t_list *stack_b, t_list *target_a)
{
	...
	if (target_a->next && find_target_to_sort(stack_a, target_a) 
	== target_a->next)
		return (0);
	if (!target_a->next && find_target_to_sort(stack_a, target_a) 
	== stack_a)
...
int	cal_dist(t_list *stack_a, t_list *stack_b, t_list *target_a)
{
	...
	if (find_b(stack_a, stack_b, target_a) == 0)
		return (-1);
	return (traverse_dist_both(stack_a, target_a, stack_b, find_b(stack_a, stack_b, target_a)));
...
t_list	*find_least_move(t_list *stack_a, t_list *stack_b)
{
	...
	if (!fake_list)
		...
		while (fake)
		{
			...
			if (dist != -1 && (target == 0 || dist < 
				cal_dist(stack_a, stack_b, target)))
...
int	check_swap(t_list *stack_a)
{
	...
	&& stack_a->next && show_int(stack_a->next) == high_num(stack_a))
		return (1);
	if (find_target_to_sort(stack_a, stack_a) 
	== stack_a->next)
		return (1);
	return (0);
}
...
t_list	*find_close_order(t_list *stack_a, t_list *top_b, t_list *high_num, t_list *low_num)
{
	t_list	*result;
	t_list	*top_a;
...
void	one_move_op(t_list **stack_a, t_list **stack_b,
		 t_list *target_push_b, t_list *move_b)
{
	...
	if ((op_b == &op_rotate_b && op_a == &op_rotate_a) ||
	 (op_b == &op_reverse_b && op_a == &op_reverse_a))
	{
		if (op_b == &op_rotate_b)
...
void	one_move_op_all(t_list **stack_a, t_list **stack_b,
...
int	sort_iter(t_list **stack_a, t_list **stack_b, t_list *high_num, t_list *low_num)
{
	...
	if (!find_unorder(*stack_a, *stack_a))
		return (0);
//	if (*stack_b && !find_unorder(*stack_a, *stack_a))
...
//	if (target_push_b != *stack_a || move_b != *stack_b)
//	{
////		if (check_correct_pos(*stack_a, *stack_a) == 1 && show_int(*stack_a) > show_int(*stack_b) 
////		&& check_correct_pos(*stack_a, last_a) == 1 && show_int(last_a) < show_int(*stack_b)) 
////		{
////			ft_printf("b= %d\ntop_a= %d\nlast_a= %d\n", show_int(*stack_b), show_int(*stack_a), show_int(last_a));
////			op_push_a(stack_a, stack_b, 1);
////			return (1);
////		}
//		if (find_close_order(*stack_a, *stack_b, high_num, low_num) == *stack_a 
//		&& check_correct_pos(*stack_a, ft_lstlast(*stack_a)))
//		{
//			op_push_a(stack_a, stack_b, 1);
//			return (1);
//		}
...
t_list	*find_few(t_list *stack_a, t_list *stack_b)
{
	...
	while (stack_b)
	{
		...
		if (temp[0] && temp[1] && result)
		{
			...
			if (traverse_dist_both(stack_a, temp[0], top_b, stack_b)
			< traverse_dist_both(stack_a, temp[1], top_b, result))
				result = stack_b;
		}
		stack_b = stack_b->next;
	}
...
void	sixth_algor(t_list **stack_a, t_list **stack_b)
{
	...
	max = find_max_number(*stack_a);
	while (1)
	{
		if (sort_iter(stack_a, stack_b, max, min) == 0)
			break ;
	}
//	if (*stack_b)
//	{
//		easy_rotate_all(stack_a, stack_b, 
//		find_max_number(*stack_a), find_max_number(*stack_b));
//		while (*stack_b)
//		{
//			if (show_int(ft_lstlast(*stack_a)) > show_int(*stack_b) 
//			&& show_int(max) > show_int(ft_lstlast(*stack_a)))
//				op_reverse_a(stack_a, stack_b, 1);
//			else
//				op_push_a(stack_a, stack_b, 1);
//		}
...
//	easy_rotate('a', stack_a, min, 1);
	while (*stack_b)
	{
		...
		easy_rotate_all(stack_a, stack_b, find_closest(move_b, *stack_a), move_b);
		op_push_a(stack_a, stack_b, 1);
	}
	easy_rotate('a', stack_a, min, 1);
	return ;
